        ...
        try:
            word = await session.execute(
                select(Word.eng, Word.rus).where(
                    or_(Word.eng == words, Word.rus == words), Word.user_id == user_id
                )
            )
            return list(word)[0]
        except Exception as e:
            print(
                f"Ошибка {e} в функции get_word_info. Входные данные: tg_id={tg_id}, words={words}"
            )
...
async def set_words(tg_id, words):
    async with async_session() as session:
        try:
            ...
            print(
                f"Ошибка {e} в функции set_words. Входные данные: tg_id={tg_id}, words={words}"
            )
...
async def delete_words(tg_id, word_del):
    async with async_session() as session:
        try:
            ...
            print(
                f"Ошибка {e} в функции delete_words. Входные данные: tg_id={tg_id}, word_del={word_del}"
            )
    ...
    async with async_session() as session:
        try:
            ...
            check_word = await session.scalar(
                select(Word.eng).where(
                    Word.rus.ilike(word_question), Word.user_id == user_id
                )
            )
            ...
            print(
                f"Ошибка {e} в функции check_rus_answer. Входные данные: tg_id={tg_id}, word_question={word_question}, user_answer={user_answer}"
            )
...
async def check_eng_answer(tg_id, word_question, user_answer):
    async with async_session() as session:
        try:
            user_id = await session.scalar(select(User.id).where(User.tg_id == tg_id))
            check_word = await session.scalar(
                select(Word.rus).where(
                    Word.eng.ilike(word_question), Word.user_id == user_id
                )
            )
            right_answer = check_word
            return True if right_answer == user_answer else False
            ...
            print(
                f"Ошибка {e} в функции check_eng_answer. Входные данные: tg_id={tg_id}, word_question={word_question}, user_answer={user_answer}"
            )
...
async def add_past_word(tg_id, add_word):
    async with async_session() as session:
        try:
            ...
            print(
                f"Ошибка {e} в функции add_past_word. Входные данные: tg_id={tg_id}, words={add_word}"
            )
...
async def change_user_mode(tg_id, state):
    async with async_session() as session:
        try:
            user_id = await session.scalar(select(User.id).where(User.tg_id == tg_id))
            await session.execute(
                update(Mode).where(Mode.user_id == user_id).values(state=state)
            )
            await session.commit()
        except Exception as e:
            print(
                f"Ошибка {e} в функции change_user_mode. Входные данные: tg_id={tg_id}"
            )
...
async def check_user_mode(tg_id):
    async with async_session() as session:
        try:
            user_id = await session.scalar(select(User.id).where(User.tg_id == tg_id))
            cur_state = await session.scalar(
                select(Mode.state).where(Mode.user_id == user_id)
            )
            return cur_state
        except Exception as e:
            print(
                f"Ошибка {e} в функции check_user_mode. Входные данные: tg_id={tg_id}"
            )
